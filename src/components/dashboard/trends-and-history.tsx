...
import { formatLearningForDisplay, isAutoGeneratedLearning } from "@/lib/learning-display";
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Charts appear after a few weeks of logged data.</CardDescription>
        </CardHeader>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-xs leading-relaxed sm:text-sm">
          Period totals by week — not live follower counts.
        </CardDescription>
      </CardHeader>
...
function hasDisplayableLearning(report: WeeklyReport): boolean {
  const locked = report.locked_findings?.trim();
  if (locked) return true;
  const learning = report.learning?.trim();
  if (!learning) return false;
  return !isAutoGeneratedLearning(learning);
}

export function LearningsLog({ history }: { history: WeeklyReport[] }) {
  const withLearnings = [...history].reverse().filter(hasDisplayableLearning);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Your saved notes and decisions from past weeks.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {withLearnings.map((r) => {
            const learning = r.learning?.trim();
            const showLearning = learning && !isAutoGeneratedLearning(learning);
            return (
              <div key={r.week_start} className="border-l-2 border-primary-700 pl-4">
                ...
                {showLearning && (
                  <p className="mt-1 text-sm">{formatLearningForDisplay(learning)}</p>
                )}
                {r.locked_findings && (
                  <p className="mt-2 text-sm text-muted-foreground">
                    <span className="font-medium text-foreground">Locked in:</span>{" "}
                    {formatLearningForDisplay(r.locked_findings)}
                  </p>
                )}
              </div>
            );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Period totals for each logged week.</CardDescription>
      </CardHeader>
          ...
          <tbody>
            {rows.map((r) => {
              ...
              const displayLearning =
                learning && !isAutoGeneratedLearning(learning)
                  ? formatLearningForDisplay(learning)
                  : "—";
              return (
                <tr key={r.week_start} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td className="max-w-xs truncate p-3 text-muted-foreground">{displayLearning}</td>
                </tr>
              );
            })}
          </tbody>
